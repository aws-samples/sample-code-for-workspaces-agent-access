# MCP Redirection Demo Agent

Demonstrates **MCP Redirection** (forwarded tools) on Amazon WorkSpaces
...
This demo requires a fleet built with MCP Redirection enabled. Build one with:
...
4. Writes `mcp_redirection_report.txt` combining the above, then reads it back to verify (forwarded filesystem).
...
> The streaming URL must be for the redirection-enabled fleet/stack
> (`MCPRedirect` / `MCPRedirectStack` — the defaults created by
> `setup_mcp_redirection.sh`, which prints this exact command on completion).
...
> `config.json` at the redirection fleet.

## Structure

```
mcp_redirection_demo/
...
├── skills/
│   └── mcp-redirection-skill.json  # How to use the forwarded tools
├── logs/             # Runtime logs
...
- If the agent reports no `forwarded___` tools, the streaming URL is for a
  non-redirection fleet, or the stack is missing `FORWARD_MCP_TOOLS: ENABLED`.
